...
function preload(){
  ...
  backgroundImage = loadImage("jungle.png")//Use a wider image for background
...
function setup() {
 ...
 // camera.position.x=100;
  ...
  bg = createSprite(200,200,600*10,500);//background can't be used as a variable//change1
  ...
  //monkey.addImage("standing",monkeyImage2);//change2
...
function draw() {
  ...
  if (gameState === PLAY){
    ...
    if(camera.position.x%200==0){//change3
  ...
  textSize(20);
  fill("black");
  text("Lives: "+Lives,camera.position.x,90);
...
function bananas(){
  if (frameCount % 80 === 0){
    banana = createSprite(camera.position.x+100,150,20,20);//change 5
...
function obstacles(){
  if(frameCount % 300 === 0){
    obstacle = createSprite(camera.position.x+100,350,25,25);//change 6
